...
namespace HurricaneVR.Framework.Core.Grabbers
{
    public class HVRHandGrabber : HVRGrabberBase
    {
        ...
        private bool UpdateHolding()
        {
            ...
            {
                switch (GrabbedTarget.GrabTrigger)
                {
                    case HVRGrabTriggerOverride.ManualRelease:
                        return true;
                    case HVRGrabTriggerOverride.Active:
                        grabTrigger = HVRGrabTrigger.Active;
                        break;
                    case HVRGrabTriggerOverride.Toggle:
        ...
        protected virtual bool CanGrabFromSocket(HVRSocket socket)
        {
            ...
            return socket.GrabDetectionType == HVRGrabDetection.Socket && socket.GrabbedTarget && socket.CanGrabbableBeRemoved;
        }
        ...
        private void SetToggle(HVRGrabbable grabbable)
        {
            ...
            {
                if (grabbable.GrabTrigger == HVRGrabTriggerOverride.Toggle)
                {
                    toggle = true;
                }
            }
        ...
        private Vector3 FindClosestPoint(HVRGrabbable grabbable)
        {
            ...
            foreach (var gc in grabbable.Colliders)
            {
                ...
                {
                    if (!gc.Raycast(new Ray(anchor, gc.bounds.center - anchor), out var hit, Vector3.Distance(gc.bounds.center, anchor)))
                    {
                        continue;
                    }
