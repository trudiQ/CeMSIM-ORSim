...
namespace HurricaneVR.Framework.Shared.HandPoser
{
    ...
    public class HVRPhysicsPoser : MonoBehaviour
    {
        ...
        private bool Validate()
        {
            ...
            {
                Debug.LogWarning($"ClosedPose Finger Count is {ClosedPoseData.Fingers.Length} expected {Hand.Fingers.Length}");
                return false;
            }

            if (OpenPoseData.Fingers.Length != Hand.Fingers.Length)
            {
                Debug.LogWarning($"OpenPose Finger Count is {ClosedPoseData.Fingers.Length} expected {Hand.Fingers.Length}");
                return false;
            }
