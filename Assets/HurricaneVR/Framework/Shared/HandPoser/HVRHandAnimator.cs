...
namespace HurricaneVR.Framework.Shared.HandPoser
{
    public class HVRHandAnimator : MonoBehaviour
    {
        ...
        public float[] FingerCurls => Hand.IsLeft ? HVRController.LeftFingerCurls : HVRController.RightFingerCurls;
        ...
        protected virtual void LateUpdate()
        {
            for (int i = 0; i < 5; i++)
            {
                _fingerCurls[i] = Mathf.Lerp(_fingerCurls[i], FingerCurls[i], Time.deltaTime * FingerCurlSpeed);
            }
